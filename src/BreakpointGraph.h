...
public:
    CBPGraph(const std::string& chrom, bool opensLeftVal, 
        ...
        : chromosome(chrom), opensLeft(opensLeftVal), 
          upstreamDist(upsDist), readLen(rLen), maxInsertSize(maxInsert), splitFactor(sFactor) {
    ...
    CBPGraph(CBPGraph&&) = delete;
    CBPGraph& operator=(CBPGraph&&) = delete;

    /**
     * Adds a vertex if the (proximalPos, distalPos) pair is unique.
     * If it already exists, merges attributes with the existing vertex.
     */
    void addOrUpdateVertex(int proximalPos, int distalPos, bool isSplit, const std::string& assocFragments) {
